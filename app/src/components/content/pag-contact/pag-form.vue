...
<template>
    <div class="bg-gray-50 rounded-2xl md:px-16">
        <h2 class="text-2xl font-bold mb-6">{{ $t("form-text-1") }}</h2>

        <input v-model="form.fio" type="text" :placeholder="$t('Ismingiz')" required class="form-input h-12 mb-1" />
        <p v-if="formErrors.fio" class="text-red-500 text-sm mb-4">{{ formErrors.fio[0] }}</p>

        <input v-model="form.email" type="email" :placeholder="$t('Elektron pochta')" required class="form-input h-12 mb-1" />
        <p v-if="formErrors.email" class="text-red-500 text-sm mb-4">{{ formErrors.email[0] }}</p>

        <input v-model="form.subject" type="text" :placeholder="$t('Mavzu')" required class="form-input h-12 mb-1" />
        <p v-if="formErrors.subject" class="text-red-500 text-sm mb-4">{{ formErrors.subject[0] }}</p>

        <textarea v-model="form.body" rows="4" :placeholder="$t('Xabaringizni yozing')" required
