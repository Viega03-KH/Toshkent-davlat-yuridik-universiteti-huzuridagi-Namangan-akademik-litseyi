<template>
  ...
  <div v-if="photoStore.loading" class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
    <Yutubecard />
    <div v-for="n in 2" :key="n" class="skeleton h-[30vh] w-full rounded-lg"></div>
  </div>
  ...
  <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <div v-for="item in photoStore.items" :key="item.id" class="group cursor-pointer relative" :data-aos="'zoom-in-up'"
      ...
      <img :src="item.url" loading="lazy" :alt="item.title || 'Image'"
        class="w-full h-[30vh] object-cover rounded-lg transition-transform transform" />
      <div
        class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
        <a
          class="bg-white text-gray-800 px-4 py-2 rounded-full hover:bg-blue-400 hover:text-white transition-colors group-hover:scale-[102%]">
          <Expand />
        </a>
      </div>
    </div>
  ...
  <dialog id="my_modal_13" class="modal">
    <div class="modal-box relative max-w-4xl bg-white/0 backdrop-blur-sm rounded-lg overflow-hidden p-4">
      <img :src="selectedPhoto?.url" alt="Preview" loading="lazy" class="w-full h-[80vh] rounded-md object-contain" />
      <button @click.stop="goToPrev"
        class="absolute top-1/2 left-2 transform -translate-y-1/2 bg-white text-black px-3 py-2 rounded-full hover:bg-gray-200 z-10">
        <ArrowLeft :stroke-width="0.5" />
      </button>
      <button @click.stop="goToNext"
        class="absolute top-1/2 right-2 transform -translate-y-1/2 bg-white text-black px-3 py-2 rounded-full hover:bg-gray-200 z-10">
